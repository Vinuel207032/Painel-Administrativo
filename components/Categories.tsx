...
export const Categories: React.FC<CategoriesProps> = ({ 
  ...
  const handlePrint = () => {
    ...
    const rows = filtered.map(c => `
      <tr>
        <td style="padding: 12px; border-bottom: 1px solid #eee;">
          <div style="width: 24px; height: 24px; border-radius: 50%; background-color: ${c.cor_hexadecimal};"></div>
        </td>
        <td style="padding: 12px; border-bottom: 1px solid #eee; font-weight: bold; font-size: 11px; text-transform: uppercase;">${c.nome}</td>
        <td style="padding: 12px; border-bottom: 1px solid #eee; font-size: 10px;">${c.empresas[0]?.count || 0}</td>
        <td style="padding: 12px; border-bottom: 1px solid #eee; font-size: 10px; font-weight: 800; color: #000;">${c.status}</td>
      </tr>
    `).join('');
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Relatório de Categorias da Empresa</title>
          <style>
        ...
        <body>
          ...
          <div class="header">
            <h1>Relatório de Categorias da Empresa</h1>
  ...
  return (
    <div className="p-6 bg-white rounded-3xl shadow-lg">
      <Breadcrumb items={[{ icon: Layers, label: 'Configurações', link: '#' }, { label: 'Categorias' }]} />
      <div className="flex justify-between items-center mt-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Categorias</h1>
          <p className="text-sm text-slate-500">Gerencie as categorias de parceiros do sistema.</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left text-slate-500">
          <thead className="text-xs text-slate-700 uppercase bg-slate-50">
            <tr>
              <th scope="col" className="px-6 py-3">Ícone</th>
              <th scope="col" className="px-6 py-3">Nome</th>
              <th scope="col" className="px-6 py-3">Qtd de Empresas</th>
            ...
            {paginatedData?.map(c => (
              <tr key={c.id} className="bg-white border-b hover:bg-slate-50">
                <td className="px-6 py-4">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: c.cor_hexadecimal }}>
                    {/* You might need a library to render icons from a string, or use an img tag if it's a URL */}
                    <Layers size={16} color={getContrastColor(c.cor_hexadecimal)} />
                  </div>
                </td>
                <td className="px-6 py-4 font-medium text-slate-900 whitespace-nowrap">{c.nome}</td>
                <td className="px-6 py-4">{c.empresas[0]?.count || 0}</td>
                <td className="px-6 py-4">
                  <div className={`w-3 h-3 rounded-full ${c.status === 'ATIVO' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                </td>
                <td className="px-6 py-4 text-right">
                  <button onClick={() => handleEdit(c)} className="font-medium text-blue-600 hover:underline">
                    <Edit3 size={18} />
                  </button>
                </td>
              </tr>
